...
var products = function (state, action) {
    if (state === void 0) { state = exports.initialState.products; }
    var product = action.payload.product, id = action.id.id;
    switch (action.type) {
        case actionTypes_1.ADD_TO_CART:
            if (state.findIndex(function (item) { return item.objectID === product.objectID; }) !== -1) {
                return state;
            }
            return __spreadArrays(state, [product]);
        case actionTypes_1.REMOVE_FROM_CART:
            console.log('state is', state);
            console.log('id is', id);
            return state.filter(function (item) { return item.objectID !== id; });
        default:
            return state;
    }
};
